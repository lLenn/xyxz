...
if (Session :: get_user_id() && !is_null(Request::post("user_id")))
{
	...
	if($um->get_user()->is_child(Request::post("user_id")))
	{
		...
		echo '<h4 class="small_title"><img id="remove_end_game_ajax" style="left: -22px; top: 1px; border: 0; position: relative;" src="' . Path::get_url_path() . '/layout/images/buttons/remove.png"/><span style="position: relative; left: -14px;">' . Language::get_instance()->translate(779) . '</span></h4>' . "\n";
		echo $gm->get_end_game_manager()->get_renderer()->get_table(RightManager::READ_RIGHT, false, false, false);
	}
}
