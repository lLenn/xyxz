...
class FormValidator extends HTML_QuickForm
{
    ...
    public function __construct($form_name, $method = 'post', $action = '', $target = '', $attributes = null, $trackSubmit = true)
    {
        ...
        $this->registerElementType(
            'select_language',
            $dir . 'Element/select_language.php',
            'HTML_QuickForm_Select_Language');
        $this->registerElementType(
            'upload_or_create',
            $dir . 'Element/upload_or_create.php',
            'HTML_QuickForm_upload_or_create');
        $this->registerElementType(
            'element_finder',
            $dir . 'Element/element_finder.php',
            'HTML_QuickForm_element_finder');
        $this->registerElementType(
            'advanced_element_finder',
            $dir . 'Element/advanced_element_finder.php',
            'HTML_QuickForm_advanced_element_finder');

        $registration = CommonDataManager :: get_registration('repository');
        if ($registration instanceof Registration && $registration->is_active())
        {
            $this->registerElementType(
                'image_selecter',
                Path :: namespace_to_full_path('repository') . 'php/lib/form/element/image_selecter.php',
                '\repository\HTML_QuickForm_image_selecter');
        }

        $this->registerElementType(
            'user_group_finder',
            $dir . 'Element/user_group_finder.php',
            'HTML_QuickForm_user_group_finder');
        $this->registerElementType(
            'option_orderer',
            $dir . 'Element/option_orderer.php',
        ...
        $this->registerElementType(
            'style_submit_button',
            $dir . 'Element/style_submit_button.php',
            'HTML_QuickForm_stylesubmitbutton');
        $this->registerElementType(
            'style_reset_button',
            $dir . 'Element/style_reset_button.php',
            'HTML_QuickForm_styleresetbutton');
        $this->registerElementType(
            'checkbox',
            $dir . 'Element/extended_checkbox.php',
        ...
        $this->registerRule('number_compare', null, 'HTML_QuickForm_Rule_NumberCompare', $dir . 'Rule/NumberCompare.php');
        $this->registerRule('html', null, 'HTML_QuickForm_Rule_HTML', $dir . 'Rule/HTML.php');
        $this->registerRule(
            'username_available',
            null,
            'HTML_QuickForm_Rule_UsernameAvailable',
            $dir . 'Rule/UsernameAvailable.php');
        ...
        $registration = CommonDataManager :: get_registration('repository\quota');
        if ($registration instanceof Registration && $registration->is_active())
        {
            $this->registerRule(
                'disk_quota',
                ...
                Path :: namespace_to_full_path('repository\quota') . 'php/lib/form/rule/DiskQuota.php'
            );
        }
        $this->registerRule('jquery_date', null, 'HTML_QuickForm_Rule_JqueryDate', $dir . 'Rule/JqueryDate.php');
        $this->registerRule('jquery_time', null, 'HTML_QuickForm_Rule_JqueryTime', $dir . 'Rule/JqueryTime.php');

        $this->addElement(
            'html',
            '<script type="text/javascript" src="' . Path :: get_web_common_libraries_path() .
...
EOT;
        ...
        $element_template[] = '{label}<!-- BEGIN required --><span class="form_required"><img src="' .
             Theme :: get_common_image_path() . 'action_required.png" alt="*" title ="*"/></span> <!-- END required -->';
        ...
        HTML_QuickForm :: setRequiredNote(
            '<span class="form_required"><img src="' . Theme :: get_common_image_path() .
...
EOT;
    ...
    public function add_tabs($tabs, $selected_tab)
    {
        ...
        $this->addElement(
            ...
            ResourceManager :: get_instance()->get_resource_html(
                Path :: get(WEB_PATH) . 'common/libraries/resources/javascript/form_tabs.js'));
    }
    ...
    public function add_allowed_html_tags($full_page = false)
    {
        $html = '<br/><small><a href="#" onclick="MyWindow=window.open(' . "'" . Path :: get(WEB_LIB_PATH) .
             "html/allowed_html_tags.php?fullpage=" . ($full_page ? '1' : '0') .
             "','MyWindow','toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=500,height=600,left=200,top=20'" .
             '); return false;">' . Translation :: get('AllowedHTMLTags') . '</a></small><br />';
    ...
    /**
     */
    public function add_forever_or_timewindow($element_label = 'PublicationPeriod', $element_name_prefix = '')
    {
    ...
    public function add_resource_button()
    {
        $group[] = $this->createElement(
            ...
            'add_resource_img',
            null,
            '<img src="' . Theme :: get_common_image_path() . 'action_attachment.png" alt="' .
    ...
    public function add_progress_bar($delay = 2)
    {
        ...
        $this->updateAttributes(
            "onsubmit=\"javascript: myUpload.start('dynamic_div','" . Theme :: get_common_image_path() .
                 "action_progress_bar.gif','" . Translation :: get('PleaseStandBy') . "','" . $this->getAttribute('id') .
                 "');\"");
        $this->addElement(
            'html',
            '<script src="' . Path :: get(WEB_LIB_PATH) .
                 'libraries/resources/javascript/upload.js" type="text/javascript"></script>');
        $this->addElement(
            'html',
    ...
    /**
     * Display the form. If an element in the form didn't validate, an error message is showed asking the user to
     * complete the form.
     */
